...
static const char common_header[] =
    ...
    "#wifi{font-size:18px;display:flex;align-items:center;gap:6px;font-weight:600;}"
    "#wifi span{font-size:20px;}"
    "#wifi.connected span{color:#2e7d32;}"    /* green */
    "#wifi.nointernet span{color:#ff9800;}"   /* orange */
    "#wifi.disconnected span{color:#c62828;}" /* red */
    ...
    "    <div id='clock'>--:--:--</div>"
    "  </div>"
    "  <div id='wifi'><span>📶</span><span id='wifi-text'>--</span></div>"
    "</div>"
    ...
    "    const parts=t.split(',');"
    "    document.getElementById('clock').innerText=(parts[0]||'--:--:--').trim();"
    "    document.getElementById('date').innerText=(parts[1]||'').trim();"
    "  }).catch(()=>{});"
    "}"
    "function updateWifi(){"
    ...
    "    const wifi=s.wifi, net=s.internet;"
    "    if(wifi==='connected' && net==='available'){"
    "      el.className='connected';icon.textContent='📶';text.textContent='Connected';"
    "    } else if(wifi==='connected' && net==='unavailable'){"
    "      el.className='nointernet';icon.textContent='⚠️';text.textContent='No Internet';"
    "    } else {"
    "      el.className='disconnected';icon.textContent='❌';text.textContent='Disconnected';"
    "    }"
    "  }).catch(()=>{"
    "    const el=document.getElementById('wifi');"
    "    const icon=el.querySelector('span');"
    "    const text=document.getElementById('wifi-text');"
    "    if(el&&icon&&text){el.className='disconnected';icon.textContent='❌';text.textContent='Error';}"
    "  });"
    "}"
